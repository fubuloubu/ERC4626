...
All ERC-4626 SHOULD implement ERC-20's optional metadata extensions.
...
- management fee: fee charged on deposited assets while in Vault.
- withdrawal fee: fee charged on exit to the Vault.

Please note that this standard does not consider any more complex fee structures than the above.
...
Note that any discrepency between `shares * pricePerShare` and `assets` SHOULD be considered slippage
...
Note that any discrepency between `assets / pricePerShare` and `shares` SHOULD be considered slippage
...
MUST return `2 ** 256 - 1` to specify there is no maximum amount of assets that may be withdrawn.
...
MUST return `2 ** 256 - 1` to specify there is no maximum amount of shares that may be redeemed.
...
The Vaults are opinionated on a default deposit/withdraw flow because it gives integrators a default pattern to implement.
Special use cases can be overloaded on these methods by including additional logic such as requesting withdrawals.
